...
/*   Updated: 2023/02/14 18:53:20 by lopezz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static void	*init_xpm(t_game *game, char *str)
{
	char *route;
	void	*img;

	route = ft_strjoin(XPM_ROUTE, str);
	img = mlx_xpm_file_to_image(game->mlx, route, &game->img_width, &game->img_height);
	if (!img)
	{
		ft_putstr_fd(RED"Failed to load XPM files"RESET, 1);
		exit(EXIT_ERROR);
	}
	free(route); //comprobar
	return(img);
}
...
{
	//game->map.map_img = init_xpm(game, "mapa.xpm");
